...
namespace FlexibleCalculator.Tests
{
    public class RpnCalculatorTests
    {
        ...
        [TestCase("(2+3)/5", ExpectedResult = 1)]
        public double Calculate_Expression_ShouldReturnCalculatedExrepssion(string input)
        {
            var tokenParser = new TokenParser();
            var rpnAlgo = new ReversePolishNotationAlgo(
                new IOperation[] { new PlusOperation(), new MinusOperation(), new MultiplicationOperation(), new DivisionOperation() });
